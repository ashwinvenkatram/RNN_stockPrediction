...
def plotly_graph(y_pred, y_test, r2_score):
    dates = processed['df']['date']
    openData = processed['df']['Open']
    highData = processed['df']['High']
    lowData = processed['df']['Low']
    closeData = processed['df']['Close']

    fig = make_subplots(
        rows=2, cols=1,
        shared_xaxes=True,
        vertical_spacing=0.05,
        subplot_titles=("Equity", f"{ticker}-Candlestick Chart")
    )

    fig.add_trace(
        go.Scatter(x=dates, y=closeData),
        row=1, col=1
    )

    # http://127.0.0.1:50979/
    fig.add_trace(
        go.Candlestick(x=dates, open=openData, high=highData,low=lowData,close=closeData),
        row=2, col=1
    )

    fig.add_trace(
        go.Scatter(x=dates, y=closeData + 30, mode="markers", marker=dict(color='rgba(244,96,96, 1)', size=8, symbol="triangle-down")),
        row=2, col=1
    )

    fig.add_trace(
        go.Scatter(x=dates, y=openData - 30, mode="markers", marker=dict(color='rgba(50,184,50,1)', size=8, symbol="triangle-up")),
        row=2, col=1
    )
    ...
    graphPath = "file://" + os.getcwd() + "/iframe_figures/figure_0.html"
    webbrowser.open(graphPath)
...
def slidingWindowPred(model, dataset):
    ...
    plotly_graph(y_predInv, y_testInv, r2_score)
...
if __name__ == "__main__":
    ...
    print("before sliding window")
    slidingWindowPred(model, processed)
